...
with open('credit_Model.pkl', 'rb') as model_file:
...
#for your previous pay status 
pay_status_sept = st.sidebar.slider("PAY_0 (Repayment status in September)", -2, 8, 0)
pay_status_aug = st.sidebar.slider("PAY_2 (Repayment status in August)", -2, 8, 0)
pay_status_jul = st.sidebar.slider("PAY_3 (Repayment status in July)", -2, 8, 0)
pay_status_jun = st.sidebar.slider("PAY_4 (Repayment status in June)", -2, 8, 0)
pay_status_may = st.sidebar.slider("PAY_5 (Repayment status in May)", -2, 8, 0)
pay_status_apr = st.sidebar.slider("PAY_6 (Repayment status in April)", -2, 8, 0)


#bill paymentof last 6 monthes 
...
if st.sidebar.button("Predict"):
    ...
    st.markdown(
    f'<h3 style="background-color:#6dc0e3;padding:10px;border-radius:10px;">Prediction Result</h3>',
    unsafe_allow_html=True
    )
    if predicted_default[0] == 1:
        st.write("The client may default on their credit card.")
    else:
        st.write("The client is unlikely to default on their credit card.")
